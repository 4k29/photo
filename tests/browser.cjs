...
(async()=>{
 ...
 try{
  await p.route('**/vendor/exifr.js',route=>route.fulfill({status:404,body:'Not found'}));
  await p.route('https://cdn.jsdelivr.net/npm/exifr@7.1.3/dist/full.umd.js',route=>route.fulfill({path:path.join(root,'vendor/exifr.js'),contentType:'text/javascript'}));
  ...
  const metadata=await p.evaluate(()=>[formatExif({ExposureTime:.6}).exposure,formatExif({ExposureTime:1/250,FNumber:1.8,FocalLength:35,ISO:100,DateTimeOriginal:'2026:09:14 10:30:00'})]);assert.equal(metadata[0],'0.6 s');assert.match(metadata[1].exposure,/1\/250 s/);assert.equal(metadata[1].date,'2026.09.14 10:30');
  ...
  assert.deepEqual(errors,[]);console.log('PASS: real browser EXIF fallback/read/isolation/absence, nine watermark styles, five collage layouts, all aspect ratios, ordering, focal point, JPEG/PNG export dimensions, mobile overflow, no browser exceptions.');
